      ...
      case 'order': {
        this.orderService.fetchAllOrderForAdmin().subscribe((data: any) => {
          ...
          const formattedOrderData = filteredData.map((item: any) => ({
            ...
            orderUpdatedAt: item.updated_at,
            isCancelled: item.is_cancelled ? "Yes" : "No",
            ...
            orderItems: item.order_items.map((orderItem: any) => ({
              ...
              priceAtOrder: orderItem.price_at_order,
              subTotal: orderItem.sub_total
            })).map((orderItem: any) => `Product: ${orderItem.productName}, Quantity: ${orderItem.quantity}, Price: ${orderItem.priceAtOrder}, SubTotal: ${orderItem.subTotal}`).join('; ') // Flatten order items as a string
          }));
